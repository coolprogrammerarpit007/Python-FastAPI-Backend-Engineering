...
def download_single_image(url:str,img_num:int) -> Path:
    print(f"Downloading Path {url}...")
    ts = int(time.time())
    url = f"{url}?ts={ts}" # to avoid caching issue
    response = requests.get(url,timeout=10,allow_redirects=True)
    response.raise_for_status()
    
    filename = f"image_{img_num}.jpg"
    download_path = ORIGINAL_DIR/filename
    with download_path.open("wb") as f_hand:
...
def process_single_image(url:str) -> Path:
...
async def main():
    ...
    process_total_time = finished_time - proc_start_time
    
    total_time = dl_total_time + process_total_time
    
    print(
        f"\nDownloaded {len(img_paths)} images in: {dl_total_time:.2f} seconds. {(dl_total_time / total_time) * 100:.2f}% of total time",
    )
    print(
        f"Processed {len(processed_paths)} images in: {process_total_time:.2f} seconds. {(process_total_time / total_time) * 100:.2f}% of total time",
    )
    print(
